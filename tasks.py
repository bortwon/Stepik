...
# здесь продолжайте программу
res = [list(map(int, i.split())) for i in lst_in]
res = list(zip(*res))
random.shuffle(res)
res = list(zip(*res))
for i in res:
...
# ----------------------------------------------------------------------------------------------------------------------
# Подвиг 6. Вводятся имена студентов в одну строчку через пробел. Требуется случайным образом выбрать трех студентов
# из этого списка, используя функцию sample. (Полагается, что в исходном списке более трех студентов). Результат
# вывести на экран в одну строчку через пробел.
#
# Sample Input:
